...
# Script to build and run the Test only for Server record - DELETE THIS MCP Server locally in Docker
...
IMAGE_NAME="test-only-for-server-record-delete-this-mcp-server"
CONTAINER_NAME="test-only-for-server-record-delete-this-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Test only for Server record - DELETE THIS MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Test only for Server record - DELETE THIS MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Test only for Server record - DELETE THIS MCP Server is running!${NC}"
echo
